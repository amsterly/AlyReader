...
package aly.com.alyreader.widgets;
...
import aly.com.alyreader.utils.GlideCircleTransform;


public class PlayerDiscView extends RelativeLayout {

    private static final int NEEDLE_ANIMATOR_TIME = 350;
    private static final float NEEDLE_ROTATE_CIRCLE = -20.0f;
...
//    }
    ...
    private void startDiscAnimator(float animatedValue) {
        mDiscLayoutAnimator = ObjectAnimator.ofFloat(mAlbumCover, "rotation", animatedValue, 360 + animatedValue);
    ...
    private void reverseDiscAnimator() {
        mDiscLayoutAnimator = ObjectAnimator.ofFloat(mAlbumCover, "rotation", mDiscLayoutAnimatorValue, 360);
        mDiscLayoutAnimator.addUpdateListener(new AnimatorUpdateListener() {
    ...
    public void loadAlbumCover(String imageUrl) {
        Glide.with(mContext).load(imageUrl).transform(new GlideCircleTransform(mContext)).placeholder(R.drawable.thumb).into(mAlbumCover);
//        ImageLoader.getInstance().displayImage(imageUrl, mAlbumCover,
//                ImageLoaderHelper.getInstance
//                        (mContext).getDisplayOptions(100));
    }

    public void loadAlbumCover(int imageUrl) {
        Glide.with(mContext).load(imageUrl).transform(new GlideCircleTransform(mContext)).placeholder(R.drawable.thumb).into(mAlbumCover);
//        ImageLoader.getInstance().displayImage(imageUrl, mAlbumCover,
//                ImageLoaderHelper.getInstance
//                        (mContext).getDisplayOptions(100));
